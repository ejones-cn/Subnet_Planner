...
if exist "%PFX_FILE%" (
    ...
    if not "%SIGNTOOL%"=="" (
        set /p "PFX_PASSWORD=请输入证书密码: "
        echo.
        echo [信息] 正在签名安装包...
        "%SIGNTOOL%" sign /fd SHA256 /f "%PFX_FILE%" /p "!PFX_PASSWORD!" /t "http://timestamp.digicert.com" "%SETUP_EXE%"
        ...
        echo.
        echo [信息] 正在签名主程序...
        "%SIGNTOOL%" sign /fd SHA256 /f "%PFX_FILE%" /p "!PFX_PASSWORD!" /t "http://timestamp.digicert.com" "%NUITKA_DIR%\SubnetPlanner.exe"
