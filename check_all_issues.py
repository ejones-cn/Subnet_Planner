        ...
        def visit_FunctionDef(self, node):
            ...
            for arg in node.args.args:
                if arg.arg not in used_vars and arg.arg != 'self':  # 忽略self参数
                    issues.append(f"1. 第 {node.lineno} 行: 函数 {node.name} 有未使用的参数 {arg.arg}")
...
def check_repeated_lines():
    ...
    for line_nums in repeated_lines[:10]:  # 只显示前10个
        ...
        issues.append(f"2. 重复的代码行: 行 {', '.join(map(str, line_nums))}: {line_content[:60]}...")
...
def check_long_lines():
    ...
    for i, line in enumerate(lines, 1):
        if len(line) > 120:  # 超过120个字符的长行
            issues.append(f"3. 第 {i} 行: 长行 ({len(line)} 字符)")
    ...
    for i, line in enumerate(lines, 1):
        for pattern in commented_code_patterns:
            if re.match(pattern, line):
                issues.append(f"4. 第 {i} 行: 注释掉的代码")
                break
    ...
    for i, line in enumerate(lines, 1):
        if not line.strip():
            ...
            if current_empty >= 3:  # 超过3个连续空行
                issues.append(f"5. 第 {start_line}-{start_line+current_empty-1} 行: {current_empty} 个连续空行")
            current_empty = 0
