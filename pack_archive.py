...
def main() -> int:
    """主函数"""
    # 读取配置文件
    config_path: str = os.path.join(os.getcwd(), 'pack_archive_config.json')
    ...
    # 生成压缩包文件名
    zip_filename: str = f"{filename_prefix}_V{version}_{timestamp}.zip"
    zip_path: str = os.path.join(destination_dir, zip_filename)
    ...
    try:
        ...
        # 清理之前的临时目录
        temp_dir_pattern: str = os.path.join(os.getcwd(), "temp_pack_*")
        import glob
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            for root, dirs, files in os.walk(source_path):
                # 计算当前目录的相对路径
                relative_dir: str = os.path.relpath(root, source_path)
                ...
                while current_path != source_path:
                    parent_dir: str = os.path.basename(current_path)
                ...
                for file in files:
                    ...
                    relative_file: str = os.path.relpath(file_path, source_path)
                    
                    # 检查文件是否在排除目录中
                    file_dir: str = os.path.dirname(file_path)
                    file_in_excluded_dir: bool = False
                    while file_dir != source_path:
                        file_dir_name: str = os.path.basename(file_dir)
                        if file_dir_name in exclude_items:
