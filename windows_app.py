        ...
        try:
            ...
            if data_source["main_name"] == "切分段信息":
                ...
                if (
                    ...
                    and self.chart_data is not None
                    and isinstance(self.chart_data, dict)
                ):
                ...
                try:
                    ...
                    if not canvas_capture_success:
                        ...
                        for i, network in enumerate(split_networks):
                            ...
                            draw.text(
                                (chart_x + 30, segment_text_y), segment_text, fill="#ffffff", font=bold_text_font
                            )
                            draw.text(
                                (address_x, address_text_y), address_text, fill="#ffffff", font=bold_text_font
                            )
                        ...
                        # 图例标题
                        legend_title = "图例说明:"
                        title_bbox = draw.textbbox((0, 0), legend_title, font=bold_text_font)
                        title_text_y = get_centered_y(legend_y, bar_height, title_bbox, bold_text_font)
                        draw.text((legend_x, title_text_y), legend_title, fill="#ffffff", font=bold_text_font)
                        legend_y += 100

                        # 图例项：父网段
                        parent_legend_text = "父网段"
                        parent_legend_x = legend_x
                        parent_legend_y = legend_y
                        draw.rectangle(
                            [parent_legend_x, parent_legend_y - 15, parent_legend_x + 60, parent_legend_y + 15],
                            fill="#636e72",
                            outline=None,
                            width=0,
                        )
                        draw.text(
                            (parent_legend_x + 80, parent_legend_y - 15),
                            parent_legend_text,
                            fill="#ffffff",
                            font=bold_text_font,
                        )
                        ...
                        # 图例项：切分网段
                        split_legend_text = "切分网段"
                        split_legend_x = legend_x + 300
                        split_legend_y = legend_y
                        draw.rectangle(
                            [split_legend_x, split_legend_y - 15, split_legend_x + 60, split_legend_y + 15],
                            fill="#4a7eb4",
                            outline=None,
                            width=0,
                        )
                        draw.text(
                            (split_legend_x + 80, split_legend_y - 15),
                            split_legend_text,
                            fill="#ffffff",
                            font=bold_text_font,
                        )
                        ...
                        # 图例项：剩余网段
                        remaining_legend_text = "剩余网段"
                        remaining_legend_x = legend_x + 600
                        remaining_legend_y = legend_y
                        draw.rectangle(
                            [remaining_legend_x, remaining_legend_y - 15, remaining_legend_x + 60, remaining_legend_y + 15],
                            fill="#5e9c6a",
                            outline=None,
                            width=0,
                        )
                        draw.text(
                            (remaining_legend_x + 80, remaining_legend_y - 15),
                            remaining_legend_text,
                            fill="#ffffff",
                            font=bold_text_font,
                        )

                        # 将PIL图像转换为reportlab可用的图像数据
                        image_buffer = BytesIO()
                        pil_image.save(image_buffer, format='PNG', dpi=(300, 300), quality=95, optimize=True)
                        image_buffer.seek(0)

                        # 添加图像到PDF
                        pdf_image = RLImage(image_buffer, width=17.0 * cm, height=23.0 * cm, kind='direct')
                        elements.append(Spacer(1, 20))
                        elements.append(pdf_image)
                        elements.append(Spacer(1, 20))

                except (FileNotFoundError, IOError, OSError, ValueError, TypeError) as e:
                    print(f"处理网段分布图失败: {type(e).__name__}: {e}")
                    traceback.print_exc()
            else:
                print("未检测到有效网段分布图数据，跳过图表添加")
            ...
            if file_ext == ".json":
                ...
                if has_chart_data and has_networks:
                    ...
                    try:
                        ...
                        if not canvas_capture_success:
                            ...
                            for i, network in enumerate(remaining_networks):
                                ...
                                color = subnet_colors[color_index]
                                draw.rectangle(
                                    [chart_x, y, chart_x + bar_width, y + bar_height], fill=color, outline=None, width=0
                                )
                            ...
                            parent_label_y = get_centered_text_y(
                                legend_container_y, legend_container_height, parent_label_bbox
                            )
                            ...
                            draw.rectangle(
                                [
                                    parent_x,
                                    parent_block_y,
                                    parent_x + parent_block_size,
                                    parent_block_y + parent_block_size,
                                ],
                                fill=parent_color,
                                outline=None,
                                width=0,
                            ...
                            draw.text(
                                (parent_x + parent_block_size + 25, parent_label_y),
                                parent_label,
                                fill="#ffffff",
                                font=parent_text_font,
                            )
                            ...
                            split_label_y = get_centered_text_y(
                                legend_container_y, legend_container_height, split_label_bbox
                            )

                            draw.rectangle(
                                [split_x, split_block_y, split_x + split_block_size, split_block_y + split_block_size],
                                fill=split_color,
                                outline=None,
                                width=0,
                            )
                            draw.text(
                                (split_x + split_block_size + 25, split_label_y),
                                split_label,
                                fill="#ffffff",
                                font=split_text_font,
                            )
                            ...
                            # 精确计算垂直居中位置
                            remaining_block_y = (
                                legend_container_y + (legend_container_height - remaining_block_size) // 2
                            )
                            remaining_label_y = get_centered_text_y(
                                legend_container_y, legend_container_height, remaining_label_bbox
                            )
                            ...
                            for j, color in enumerate(legend_colors):
                                draw.rectangle(
                                    [
                                        ...
                                        remaining_block_y,
                                        remaining_x
                                        + j * (remaining_block_size + remaining_block_gap)
                            ...
                            draw.text(
                                (
                                    remaining_x
                                    + len(legend_colors) * (remaining_block_size + remaining_block_gap)
                                    + 40,
                                    remaining_label_y,
                                ),
                                remaining_label,
                        ...
                        # 使用动态计算的图像高度
                        actual_image_height = (
                            dynamic_high_res_height if 'dynamic_high_res_height' in locals() else high_res_height
                        )
                    ...
                    if hasattr(self, 'chart_data') and self.chart_data is not None:
                        print(
                            f"networks数量: {len(self.chart_data['networks']) if 'networks' in self.chart_data else 0}"
                        )
